...
(function($) {
	$.fn.availableTags = function() {
		...
		if($(joomlaSource).is(":selected")){
			available = ['title','image','text','date','category','more','column2','column3','column4','tweet','isfeatured','tags','author'];
		}

		if($(k2Source).is(":selected")){
			available = ['title','image','text','date','category','more','comments','extrafields','video','column2','column3','column4','tweet','isfeatured','tags','author','avatar']
		}
		if($(easyblogSource).is(":selected")){
			available = ['title','image','text','date','category','more','comments','video','column2','column3','column4','tweet','isfeatured','tags','author','avatar']
		}
		if($(categorySource).is(":selected")){
			available = ['title','image','text','column2','column3','column4']
		}
	}
	...
	$.fn.setSortables = function() {
		...
		// Look through the array of possible tags and filter out the ones that can't be used for that content source.
		$.each(used, function(i, val){
			 if($.inArray(val, available) < 0)
				 used = jQuery.grep(used, function(value) {
				   return value != val;
				 });
		});
	...
	$.fn.authorPanel = function() {
		$(authorPanel).parent().show();
	}
	...
	$.fn.layoutSwitch= function() {
		...
		if($("#sortable li#author").length == 1)  	{$(this).authorPanel();}
