...
class JFormFieldScripts extends JFormField
{
	...
	protected function getInput()
	{
		...
		else
		{
			...
			if ((version_compare(JVERSION, '3.2', '<=')) && (JPluginHelper::isEnabled('system', 'advancedmodules') && JRequest::getVar('option') === 'com_advancedmodules'))
			{
			...
			else if (JPluginHelper::isEnabled('system', 'advancedmodules') && JRequest::getVar('option') === 'com_advancedmodules')
			{
				$document->addScript(''.$root.'/media/mod_zentools/js/admin/scripts32-advancedmodule.js');
				$document->addScript(''.$root.'/media/mod_zentools/js/admin/sortables32-advancedmodule.js');
			}
			else if (version_compare(JVERSION, '3.2', '<='))
			{
			...
			{
				$document->addScript(''.$root.'/media/mod_zentools/js/admin/scripts32.js');
				$document->addScript(''.$root.'/media/mod_zentools/js/admin/sortables32.js');
			}
		}
		...
		<div id="easyblog-installed" style="display:none"><?php echo ZenToolsHelper::isEasyblogInstalled();?></div>
		<?php
		return ob_get_clean();
	}
}