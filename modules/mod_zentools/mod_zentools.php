...
else if ($contentSource=="4" && ZenToolsHelper::isEasyblogInstalled()) {
	require_once dirname(__FILE__) . '/includes/zeneasybloghelper.php';
}
else if ($contentSource=="5") {
	require_once dirname(__FILE__) . '/includes/zencategoryhelper.php';
}
// Parameters
...
if($scripts) {
	//if(!$zgf) {
		if(!$cache){
			...
			$document->addStyleSheet($mediaURI .'css/zentools.min.css');
			...
			$document->addScript($mediaURI .'js/zentools.js');
			...
			if($layout == "slideshow" && $slideshowTheme =="backstretch") {
				$document->addScript($mediaURI .'js/slideshow/jquery.backstretch.min.js');
			}
			...
			if($layout == "accordion") {
				if($params->get('theme') !=="default") {
					$document->addStyleDeclaration('
						#zentools'.$moduleID.' .accordion .firstitem span:before  { content: "'.$params->get('accordionSymbolClosed','\2d').'"; font-family: "fontello" } 
						#zentools'.$moduleID.' .accordion .firstitem.open span:before  {content: "'.$params->get('accordionSymbolOpen','\2d').'"; font-family: "fontello"} 
					');
				}
			}
			...
			// Style declaration for more and button
			if ($params->get('overlayMore') && !$params->get('usebtnclass')) {
				$document->addStyleDeclaration('#zentools'.$moduleID.' .zenmore a.overlaymore {font-size: '.$params->get('moresize').';color: '.$params->get('morecolor').'}');
			}
			
			if ($params->get('overlayMore')) {
				$document->addStyleDeclaration('#zentools'.$moduleID.' div.overlaymore {margin-top:'.$params->get('moretopoffset').';top:-'.$params->get('moretopoffset').'}');
			}
...
if($contentSource=="1") {
	...
	$list = ModZentoolsEasyBlogHelper::getList($params, $moduleID);
}
else if ($contentSource=="5") {
	$list = ModZentoolsCategoryHelper::getList($params, $moduleID);
}

require JModuleHelper::getLayoutPath('mod_zentools',$params->get('altlayout', 'default'));