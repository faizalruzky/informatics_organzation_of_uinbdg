...
class ModZentoolsK2Helper
{
	public static function getList($params, $id,$format = 'html')
	{
        ...
        $dateOrder     = $params->get('dateOrder', 'day-month-year');
		...
		if (count($items)) {
			...
			foreach ($items as $item) {
				...
				// Tags
				$article_tags = $model->getItemTags($item->id);
				
				$item->tag_filter = null;
				$item->tags = null;
				
				if(!empty($article_tags)) {
					$item->tag_filter = array();
					
					for ($i = 0; $i < sizeof($article_tags); $i++)
						{
							$article_tags[$i]->link = JRoute::_(K2HelperRoute::getTagRoute($article_tags[$i]->name));
							$item->tag_filter[] = $article_tags[$i]->name;
						}
					
					$item->tags = $article_tags;
				}
				...
				else
				{
					...
					$item->avatar = K2HelperUtilities::getAvatar($author->id, $author->email, $componentParams->get('userImageWidth'));
				...
				// Date Format
				if (!$translateDate) {
					$day = ($dayFormat === 'none' ? '' : '<span class="zenday">'.date($dayFormat,  (strtotime($item->created))).'</span>');
					$date = ($dateFormat === 'none' ? '' : '<span class="zendayinmonth">'.date($dateFormat,  (strtotime($item->created))).'</span>');
					$month = ($monthFormat === 'none' ? '' : '<span class="zenmonth">'.date($monthFormat,  (strtotime($item->created))).'</span>');
					$year = ($yearFormat === 'none' ? '' : '<span class="zenyear">'.date($yearFormat,  (strtotime($item->created))).'</span>');
					if ($dateOrder === 'year-month-day') {
