    ...
    public static function getList(&$params, $id, $handleImages = false)
    {
        ...
        foreach ($items as $key => $item) {
            ...
            **/
            ...
            if (!$translateDate) {
            	/*reformat day and month*/
            	if ($dayFormat === 'D' || $dayFormat === '%a') {
            		$dayFormat = '%a';
            	} elseif ($dayFormat === 'l' || $dayFormat === '%A') {
            		$dayFormat = '%A';
            	} else {
            		$dayFormat = 'none';
            	}
            	
            	if ($monthFormat === 'F' || $monthFormat === '%B') {
            		$monthFormat = '%B';
            	} elseif ($monthFormat === 'M' || $monthFormat === '%h') {
            		$monthFormat = '%h';
            	} elseif ($monthFormat === 'm' || $monthFormat === '%m') {
            		$monthFormat = '%m';
            	} else {
            		$monthFormat = 'none';
            	}
                ...
                $phpLang = str_replace('-', '_', $languageTag).'.utf8';
                setlocale(LC_ALL, $phpLang);
                $day = ($dayFormat === 'none' ? '' : '<span class="zenday">'.strftime($dayFormat,  (strtotime($item->created))).'</span>');
                $date = ($dateFormat === 'none' ? '' : '<span class="zendayinmonth">'.date($dateFormat,  (strtotime($item->created))).'</span>');
                $month = ($monthFormat === 'none' ? '' : '<span class="zenmonth">'.strftime($monthFormat,  (strtotime($item->created))).'</span>');
                $year = ($yearFormat === 'none' ? '' : '<span class="zenyear">'.date($yearFormat,  (strtotime($item->created))).'</span>');
               // $item->date = date($dateFormat,  (strtotime($item->created)));
                if ($dateOrder === 'day-year-month-day') {
                    $item->date = ($dayFormat === 'none' ? '' : $day.' ').$year.($monthFormat === 'none' ? '' : $dateSeparator).$month.($dateFormat === 'none' ? '' : $dateSeparator).$date;
                } else {
                    $item->date = ($dayFormat === 'none' ? '' : $day.' ').$date.($monthFormat === 'none' ? '' : $dateSeparator).$month.($yearFormat === 'none' ? '' : $dateSeparator).$year;
                }
            }
            else {
            ...
            **/
            ...
            {
                $article_tags = new JHelperTags;      
                $item->tag_filter = $article_tags->getItemTags('com_content.article', $item->id);
                $item->tagLayout = new JLayoutFile('joomla.content.tags');
                $item->tags = $item->tagLayout->render($item->tags->itemTags);
            }
