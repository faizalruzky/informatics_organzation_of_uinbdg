...
class ModZentoolsHelper
{
	function getList($params, $id, $handleImages = false)
	{
		...
		// Other Params
		$dayFormat      = $params->get('dayFormat', 'j');
		...
		$modalMore = $params->get('modalMore');
			...
			**/
			...
			if (!$translateDate) {
				...
				$day = ($dayFormat === 'none' ? '' : '<span class="zenday">'.strftime($dayFormat,  (strtotime($row->created))).'</span>');
				$date = ($dateFormat === 'none' ? '' : '<span class="zendayinmonth">'.date($dateFormat,  (strtotime($row->created))).'</span>');
                $month = ($monthFormat === 'none' ? '' : '<span class="zenmonth">'.strftime($monthFormat,  (strtotime($row->created))).'</span>');
                $year = ($yearFormat === 'none' ? '' : '<span class="zenyear">'.date($yearFormat,  (strtotime($row->created))).'</span>');
                if ($dateOrder === 'year-month-day') {
                    $lists[$i]->date = ($dayFormat === 'none' ? '' : $day.' ').$year.($monthFormat === 'none' ? '' : $dateSeparator).$month.($dateFormat === 'none' ? '' : $dateSeparator).$date;
                } else {
                    $lists[$i]->date = ($dayFormat === 'none' ? '' : $day.' ').$date.($monthFormat === 'none' ? '' : $dateSeparator).$month.($yearFormat === 'none' ? '' : $dateSeparator).$year;
                }
			}
			else {
